...
$order_row=$db->get_limit('order_twos', $where, '*', "{$order_ary[$order]}OrderId desc", $start_row, get_cfg('order_twos.page_count'));
	...
	<tr>
		<td height="22" class="flh_150">
			<form method="get" name="manage_log_search_form" action="index.php" onsubmit="this.submit.disabled=true;">
				<?=get_lang('order_twos.order_number');?>:<input name="OId" class="form_input" type="text" size="20" maxlength='20'>
				...
				<?=get_lang('order_twos.order_status');?>:<select name="OrderStatus">
...
<table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table" not_mouse_trBgcolor_tr='list_form_title'>
	<tr align="center" class="list_form_title" id="list_form_title">
		...
		<td width="10%" nowrap><strong><?=get_lang('order_twos.order_number');?></strong><a href="index.php?<?=$query_string_no_order;?>&order=1"><img src="../images/asc.png" /></a><a href="index.php?<?=$query_string_no_order;?>&order=2"><img src="../images/desc.png" /></a></td>
		<td width="10%" nowrap><strong>手机号</strong></td>
		<?php /*?><td width="10%" nowrap><strong><?=get_lang('ly200.email');?></strong><a href="index.php?<?=$query_string_no_order;?>&order=3"><img src="../images/asc.png" /></a><a href="index.php?<?=$query_string_no_order;?>&order=4"><img src="../images/desc.png" /></a></td><?php */?>
		<td width="8%" nowrap><strong><?=get_lang('order_twos.grand_total');?></strong><a href="index.php?<?=$query_string_no_order;?>&order=5"><img src="../images/asc.png" /></a><a href="index.php?<?=$query_string_no_order;?>&order=6"><img src="../images/desc.png" /></a></td>
		<td width="10%" nowrap><strong><?=get_lang('order_twos.order_status');?></strong><a href="index.php?<?=$query_string_no_order;?>&order=7"><img src="../images/asc.png" /></a><a href="index.php?<?=$query_string_no_order;?>&order=8"><img src="../images/desc.png" /></a></td>
	...
	<tr align="center">
		...
		<?php if(get_cfg('order_twos.del')){?><td><input type="checkbox" name="select_OrderId[]" value="<?=$order_row[$i]['OrderId'];?>"></td><?php }?>
		<td nowrap><?=htmlspecialchars($order_row[$i]['OId']);?></td>
		<td nowrap><?=$db->get_value('member',"MemberId = '{$order_row[$i]['MemberId']}'",'Phone');?></td>
		<?php /*?><td><?php if($menu['send_mail']){?><a href="javascript:void(0);" onclick="this.blur(); parent.openWindows('win_send_mail', '<?=get_lang('send_mail.send_mail_system');?>', 'send_mail/index.php?email=<?=urlencode($order_row[$i]['Email'].'/'.$order_row[$i]['ShippingFirstName'].' '.$order_row[$i]['ShippingLastName']);?>');"><?=htmlspecialchars($order_row[$i]['Email']);?></a><?php }else{?><?=htmlspecialchars($order_row[$i]['Email']);?><?php }?></td><?php */?>
		<td nowrap><?=get_lang('ly200.price_symbols').sprintf('%01.2f', ((1-$order_row[$i]['Discount'])*$order_row[$i]['TotalPrice']+$order_row[$i]['ShippingPrice'])*(1+$order_row[$i]['PayAdditionalFee']/100));?></td>
		<td nowrap><?=$order_status_ary[$order_row[$i]['OrderStatus']];?></td>
		<td nowrap><?=date(get_lang('ly200.time_format_full'), $order_row[$i]['OrderTime']);?></td>
		<td><a href="view.php?OrderId=<?=$order_row[$i]['OrderId']?>"><img src="../images/view.gif" alt="<?=get_lang('ly200.view');?>" /></a></td>
	</tr>
	<?php }?>
	<?php if(get_cfg('order_twos.del') && count($order_row)){?>
	<tr>
