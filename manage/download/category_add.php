...
check_permit('download_category', 'download.category.add');
...
if($_POST){
	...
	if(get_cfg('download.category.upload_pic')){
		if($BigPicPath=up_file($_FILES['PicPath'], $save_dir)){
			...
			$SmallPicPath=img_resize($BigPicPath, '', get_cfg('download.category.pic_width'), get_cfg('download.category.pic_height'));
		}
	}
	...
	if($UnderTheCateId==0){
		...
		$UId=get_UId_by_CateId($UnderTheCateId, 'download_category');
		$Dept=substr_count($UId, ',');
	}
	
	$db->insert('download_category', array(
			'UId'			=>	$UId,
			'Category'		=>	$Category,
	...
	get_cfg('download.category.description') && $Description=save_remote_img($_POST['Description'], $save_dir);
	$db->insert('download_category_description', array(
			'CateId'		=>	$CateId,
	...
	if(count(get_cfg('ly200.lang_array'))>1){
		add_lang_field('download_category', array('Category', 'SeoTitle', 'SeoKeywords', 'SeoDescription'));
		add_lang_field('download_category_description', 'Description');
		...
		for($i=1; $i<count(get_cfg('ly200.lang_array')); $i++){
			...
			if(get_cfg('download.category.description')){
				$DescriptionExt=save_remote_img($_POST['Description'.$field_ext], $save_dir);
				$db->update('download_category_description', "CateId='$CateId'", array(
	...
	$db->update('manage_operation_log', 'Operation="download_category_add"', array(
			'Value'	=>	$UnderTheCateId
		)
	);
	
	category_subcate_statistic('download_category');
	set_page_url('download_category', "CateId='$CateId'", get_cfg('download.category.page_url'), 0);
	
	save_manage_log('添加下载类别:'.$Category);
...
?>
<div class="header"><?=get_lang('ly200.current_location');?>:<a href="category.php"><?=get_lang('download.category_manage');?></a>&nbsp;-&gt;&nbsp;<?=get_lang('ly200.add');?></div>
...
<table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table">
	...
	<?php }?>
	<?php if(get_cfg('download.category.upload_pic')){?>
		<tr>
			<td nowrap><?=get_lang('ly200.photo');?>:</td>
			<td><input name="PicPath" type="file" size="50" class="form_input" contenteditable="false"></td>
		</tr>
	<?php }?>
	<?php if(get_cfg('download.category.dept')>1){?>
		<tr>
			<td nowrap><?=get_lang('ly200.under_the');?>:</td>
			<td><?=ouput_Category_to_Select('UnderTheCateId', $db->get_value('manage_operation_log', 'Operation="download_category_add"', 'Value'), 'download_category', 'UId="0,"', 'Dept<'.get_cfg('download.category.dept'), get_lang('ly200.select'));?></td>
		</tr>
	<?php }?>
	<?php if(get_cfg('download.category.seo_tkd')){?>
		<?php for($i=0; $i<count(get_cfg('ly200.lang_array')); $i++){?>
			<tr>
		...
		<?php }?>
	<?php }?>
	<?php if(get_cfg('download.category.description')){?>
		<?php for($i=0; $i<count(get_cfg('ly200.lang_array')); $i++){?>
			<tr>
