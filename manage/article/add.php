...
if($_POST){
	...
	if(count(get_cfg('ly200.lang_array'))>1){
		...
		for($i=1; $i<count(get_cfg('ly200.lang_array')); $i++){
			...
			$TitleExt=$_POST['Title'.$field_ext];
...
?>
<div class="header"><?=get_lang('ly200.current_location');?>:<a href="index.php?GroupId=<?=$GroupId;?>"><?=get_lang('article.group_'.$GroupId);?></a>&nbsp;-&gt;&nbsp;<?=get_lang('ly200.add');?></div>
...
<table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table">
	<?php for($i=0; $i<count(get_cfg('ly200.lang_array')); $i++){?>
		<tr> 
			...
			<td width="95%"><input name="Title<?=lang_name($i, 1);?>" type="text" value="" class="form_input" size="30" maxlength="100" check="<?=get_lang('ly200.filled_out').get_lang('ly200.title');?>!~*"></td>
		</tr>
	<?php }?>
	...
	<?php for($i=0; $i<count(get_cfg('ly200.lang_array')); $i++){?>
		<tr>
			<td nowrap><?=get_lang('ly200.contents').lang_name($i, 0);?>:</td>
			<td class="ck_editor"><textarea class="ckeditor" name="Contents<?=lang_name($i, 1);?>"></textarea></td>
		</tr>
	<?php }?>
	<tr>
		<td>&nbsp;</td>
		<td><input type="submit" value="<?=get_lang('ly200.add');?>" name="submit" class="form_button"><a href='index.php?GroupId=<?=$GroupId;?>' class="return"><?=get_lang('ly200.return');?></a><input type="hidden" name="GroupId" value="<?=$GroupId;?>" /></td>
	</tr>
</table>
</form>
