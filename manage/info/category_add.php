...
check_permit('info_category', 'info.category.add');
...
if($_POST){
	...
	if(get_cfg('info.category.upload_pic')){
		if($BigPicPath=up_file($_FILES['PicPath'], $save_dir)){
			...
			$SmallPicPath=img_resize($BigPicPath, '', get_cfg('info.category.pic_width'), get_cfg('info.category.pic_height'));
		}
	}
	...
	if($UnderTheCateId==0){
		...
		$UId=get_UId_by_CateId($UnderTheCateId, 'info_category');
		$Dept=substr_count($UId, ',');
	}
	
	$db->insert('info_category', array(
			'UId'			=>	$UId,
			'Category'		=>	$Category,
	...
	get_cfg('info.category.description') && $Description=save_remote_img($_POST['Description'], $save_dir);
	$db->insert('info_category_description', array(
			'CateId'		=>	$CateId,
	...
	if(count(get_cfg('ly200.lang_array'))>1){
		add_lang_field('info_category', array('Category', 'SeoTitle', 'SeoKeywords', 'SeoDescription'));
		add_lang_field('info_category_description', 'Description');
		...
		for($i=1; $i<count(get_cfg('ly200.lang_array')); $i++){
			...
			if(get_cfg('info.category.description')){
				$DescriptionExt=save_remote_img($_POST['Description'.$field_ext], $save_dir);
				$db->update('info_category_description', "CateId='$CateId'", array(
	...
	$db->update('manage_operation_log', 'Operation="info_category_add"', array(
			'Value'	=>	$UnderTheCateId
		)
	);
	
	category_subcate_statistic('info_category');
	set_page_url('info_category', "CateId='$CateId'", get_cfg('info.category.page_url'), 0);
	
	save_manage_log('添加文章类别:'.$Category);
...
?>
<div class="header"><?=get_lang('ly200.current_location');?>:<a href="category.php"><?=get_lang('info.category_manage');?></a>&nbsp;-&gt;&nbsp;<?=get_lang('ly200.add');?></div>
...
<table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table">
	...
	<?php }?>
	<?php if(get_cfg('info.category.upload_pic')){?>
		<tr>
			<td nowrap><?=get_lang('ly200.photo');?>:</td>
			<td><input name="PicPath" type="file" size="50" class="form_input" contenteditable="false"></td>
		</tr>
	<?php }?>
	<?php if(get_cfg('info.category.dept')>1){?>
		<tr>
			<td nowrap><?=get_lang('ly200.under_the');?>:</td>
			<td><?=ouput_Category_to_Select('UnderTheCateId', $db->get_value('manage_operation_log', 'Operation="info_category_add"', 'Value'), 'info_category', 'UId="0,"', 'Dept<'.get_cfg('info.category.dept'), get_lang('ly200.select'));?></td>
		</tr>
	<?php }?>
	<?php if(get_cfg('info.category.seo_tkd')){?>
		<?php for($i=0; $i<count(get_cfg('ly200.lang_array')); $i++){?>
			<tr>
		...
		<?php }?>
	<?php }?>
	<?php if(get_cfg('info.category.description')){?>
		<?php for($i=0; $i<count(get_cfg('ly200.lang_array')); $i++){?>
			<tr>
