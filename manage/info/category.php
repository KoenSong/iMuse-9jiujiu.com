...
<table width="100%" border="0" cellspacing="0" cellpadding="0" id="mouse_trBgcolor_table" not_mouse_trBgcolor_tr='category_list_form_title'>
	<tr align="center" class="category_list_form_title" id="category_list_form_title">
		...
		<td width="85%" align="left" nowrap style="padding-left:7px;"><strong><?=get_lang('ly200.category_name');?></strong><?php if(get_cfg('info.category.dept')>1){?>&nbsp;<img src="/manage/images/jt_<?=get_cfg('info.category.list_display')?0:1;?>.jpg" onclick="category_sub_menu(-1);" id="category_img" align="absmiddle" /><?php }?></td>
	...
	<tr align="center" class="category" id="category_<?=$m++;?>" style="display:<?=$category_row[$i]['Dept']==1?'':'none';?>;">
		<?php if(get_cfg('info.category.del')){?><td><input name="select_CateId[]" type="checkbox" value="<?=$category_row[$i]['CateId'];?>" /></td><?php }?>
	...
	<tr>
		<td colspan="4" class="bottom_act">
			<?php if(get_cfg('info.category.order')){?><input name="order_category" type="button" class="form_button" onClick="click_button(this, 'category_list_form', 'action');" value="<?=get_lang('ly200.order');?>"><?php }?>
