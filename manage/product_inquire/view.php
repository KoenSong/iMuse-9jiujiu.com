...
<table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table">
	...
	<tr>
		...
		<td><?=htmlspecialchars($product_inquire_row['Email']);?><?php if($menu['send_mail']){?>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0);" onclick="this.blur(); parent.openWindows('win_send_mail', '<?=get_lang('send_mail.send_mail_system');?>', 'send_mail/index.php?email=<?=urlencode($product_inquire_row['Email'].'/'.$product_inquire_row['FirstName'].' '.$product_inquire_row['LastName']);?>');" class="red"><?=get_lang('send_mail.send');?></a><?php }?></td>
	</tr>
	<?php if($product_inquire_row['Country']=='中国'){?>
		<tr>
			<td nowrap><?=get_lang('product_inquire.s_c_a');?>:</td>
			<td><?=htmlspecialchars($product_inquire_row['State'].$product_inquire_row['City'].$product_inquire_row['Area']);?></td>
		</tr>
		<tr>
	...
	<?php }else{?>
		...
		<tr>
			<td nowrap><?=get_lang('product_inquire.state');?>:</td>
			<td><?=htmlspecialchars($product_inquire_row['State']);?></td>
		</tr>
		<tr>
