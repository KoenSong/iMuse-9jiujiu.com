...
?>
<div class="header"><?=get_lang('ly200.current_location');?>:<a href="index.php"><?=get_lang('product_inquire.product_inquire_manage');?></a>&nbsp;-&gt;&nbsp;<?=get_lang('ly200.list');?></div>
...
<table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table" not_mouse_trBgcolor_tr='list_form_title'>
	...
	<tr align="center" class="<?=$product_inquire_row[$i]['IsRead']?'':'red_row';?>">
		...
		<td><a href="view.php?<?=$all_query_string;?>&IId=<?=$product_inquire_row[$i]['IId']?>"><img src="../images/view.gif" alt="<?=get_lang('ly200.view');?>" /></a></td>
	</tr>
	<?php }?>
	...
	<tr>
		<td colspan="20" class="bottom_act">
			...
			<input name="product_inquire_del" id="product_inquire_del" type="button" class="form_button" onClick="if(!confirm('<?=get_lang('ly200.confirm_del');?>')){return false;}else{click_button(this, 'list_form', 'list_form_action');};" value="<?=get_lang('ly200.del');?>">
