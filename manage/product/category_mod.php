...
?>
<div class="header"><?=get_lang('ly200.current_location');?>:<a href="category.php"><?=get_lang('product.category_manage');?></a>&nbsp;-&gt;&nbsp;<?=get_lang('ly200.mod');?></div>
...
<table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table">
    ...
    <tr>
			<td nowrap> 显示在导航:</td>
			<td><input name="IsNav" type="checkbox" <?=$category_row['IsNav']==1?'checked="checked"':''?> value="1" ></td>
	</tr>
    <?php }?>
	<?php
	...
	?>
		<tr>
			...
			<td><?=ouput_Category_to_Select('UnderTheCateId', get_CateId_by_UId($category_row['UId']), 'product_category', "UId='0,' and $e_where", $e_where, get_lang('ly200.select'));?></td>
		</tr>
	<?php }?>
    <?php if(get_cfg('product.category.brief_description')){?>
		<?php for($i=0; $i<count(get_cfg('ly200.lang_array')); $i++){?>
			<tr>
				<td nowrap><?=get_lang('ly200.brief_description').lang_name($i, 0);?>:</td>
				<td><textarea name="BriefDescription<?=lang_name($i, 1);?>" rows="5" cols="60" class="form_area"><?=htmlspecialchars($category_row['BriefDescription'.lang_name($i, 1)]);?></textarea></td>
			</tr>
		<?php }?>
	<?php }?>
