...
?>
<div class="header">
	<div class="float_left"><?=get_lang('ly200.current_location');?>:<a href="color.php"><?=get_lang('product.color_manage');?></a>&nbsp;-&gt;&nbsp;<?=get_lang('ly200.list');?></div>
	<?php if(get_cfg('product.color.add')){?><div class="float_right"><a href="color_add.php"><?=get_lang('ly200.add');?></a></div><?php }?>
</div>
...
<table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table" not_mouse_trBgcolor_tr='list_form_title'>
	...
	<?php
	$color_row=$db->get_all('product_color', 1, '*', 'MyOrder desc, CId asc');
	for($i=0; $i<count($color_row); $i++){
	?>
	<tr align="center">
		<td nowrap><?=($i+1)?></td>
