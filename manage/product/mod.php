...
if($_POST){
	...
	//判断是否产品已存在属于这个会员
	if($db->get_one('product',"ProId != '$ProId' and MemberId = '$MemberId'")){
		js_back('该会员已经有老师信息存在！');
	}
...
//保存课程表
	$db->delete('product_wholesale_class', "ProId='$ProId'");
...
$product_wholesale_class_row=$db->get_all('product_wholesale_class', "ProId='$ProId'", '*', 'PId asc');
...
<table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table">
        <tr>
            ...
            <td>
                ...
                <?php for($i=0,$ilen=count($teacher_row);$i<$ilen;$i++){ ?>
                	<option value="<?=$teacher_row[$i]['MemberId']?>"<?=$product_row['MemberId']==$teacher_row[$i]['MemberId']?' selected="selected"':''?>><?=$teacher_row[$i]['UserName']?> -- <?=$teacher_row[$i]['Phone']?></option>
                <?php } ?>
                </select>
            </td>
        </tr>
	...
	<?php }?>
        ...
        <tr>
			<td nowrap>评论积分:</td>
			<td><input name="Review" onkeyup="set_number(this, 0);"  type="text" value="<?=htmlspecialchars($product_row['Review']);?>" class="form_input" size="3" maxlength="50"> 用于排序优先(积分越高，排得越前)</td>
		</tr>
	...
	<?php /*?><?php if(get_cfg('product.brief_description')){?>
		...
		<?php }?>
	<?php }?><?php */?>
	<?php /*?><?php if(get_cfg('product.seo_tkd')){?>
		<?php for($i=0; $i<count(get_cfg('ly200.lang_array')); $i++){?>
			<tr>
