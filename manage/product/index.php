...
if($_POST['list_form_action']=='product_del'){
	...
	if(count($_POST['select_ProId'])){
		...
		for($i=0; $i<count($product_row); $i++){
			...
			//	}
			//}
			$db->delete('member_apply', "MemberId = '{$product_row[$i]['MemberId']}'");
			$db->delete('member', "MemberId = '{$product_row[$i]['MemberId']}'");
		}
	...
	<tr align="center">
		...
		<?php if(get_cfg('product.is_in_index') || get_cfg('product.is_hot') || get_cfg('product.is_recommend') || get_cfg('product.is_new') || get_cfg('product.sold_out')){?>
			<td class="flh_150">
                ...
                <?=(get_cfg('product.identity') && $product_row[$i]['Identity'])?get_lang('product.identity').'<br>':'';?>
                <?=(get_cfg('product.degree') && $product_row[$i]['Degree'])?get_lang('product.degree').'<br>':'';?>
                <?=(get_cfg('product.certification') && $product_row[$i]['Certification'])?get_lang('product.certification').'<br>':'';?>
