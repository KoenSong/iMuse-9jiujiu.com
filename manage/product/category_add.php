...
?>
<div class="header"><?=get_lang('ly200.current_location');?>:<a href="category.php"><?=get_lang('product.category_manage');?></a>&nbsp;-&gt;&nbsp;<?=get_lang('ly200.add');?></div>
...
<table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table">
	...
	<?php }?>
   	 	<tr>
			<td nowrap> 显示在导航:</td>
			<td><input name="IsNav" type="checkbox" value="1" ></td>
		</tr>
	<?php if(get_cfg('product.category.dept')>1){?>
		<tr>
			<td nowrap><?=get_lang('ly200.under_the');?>:</td>
			<td><?=ouput_Category_to_Select('UnderTheCateId', $db->get_value('manage_operation_log', 'Operation="product_category_add"', 'Value'), 'product_category', 'UId="0,"', 'Dept<'.get_cfg('product.category.dept'), get_lang('ly200.select'));?></td>
		</tr>
	<?php }?>
