...
$html_row=$db->get_all('download_category', $where, '*', 'MyOrder desc, CateId asc');

for($html=0; $html<count($html_row); $html++){
	$where=get_search_where_by_CateId($html_row[$html]['CateId'], 'download_category');
	...
	for($page=1; $page<=$total_pages; $page++){
		...
		include($site_root_path.'/inc/lib/download/list_lang_0.php');	//在适当的地方输出$download_list_lang_0即可
		//---------------------------------------------------------------------------------------------------------------------------------------------
