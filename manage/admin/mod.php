			...
			foreach($group as $key=>$value){
				...
				if(check_permit_act_value($cfg, $permit_act_ary)){
					...
					for($i=0; $i<count($permit_act_ary); $i++){
						$cfg[$permit_act_ary[$i]]==1 && $php_contents_config.="\$permit_Ly200Cfg{$k}['{$permit_act_ary[$i]}']=".(int)$_POST["permit_{$key}_{$permit_act_ary[$i]}"].";\r\n";
					}
				}
			}
		}
					...
					if(check_permit_act_value($cfg, $permit_act_ary)){
						...
						for($i=0; $i<count($permit_act_ary); $i++){
							if($cfg[$permit_act_ary[$i]]==1){
								$checked=(int)eval("return \$permit_Ly200Cfg{$k}[{$permit_act_ary[$i]}];")==1?'checked':'';
								echo "<input type='checkbox' value='1' name='permit_{$key}_{$permit_act_ary[$i]}' $checked>".get_lang('ly200.'.$permit_act_ary[$i]);
							}
						}
						echo ')';
					}
