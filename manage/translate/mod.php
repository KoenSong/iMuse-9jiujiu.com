...
?>
<div class="header"><?=get_lang('ly200.current_location');?>:<a href="index.php"><?=get_lang('translate.translate_manage');?></a>&nbsp;-&gt;&nbsp;<?=get_lang('ly200.mod');?></div>
...
<table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table">
	<tr> 
		...
		<td width="95%"><input name="Name" type="text" value="<?=htmlspecialchars($translate_row['Name']);?>" class="form_input" size="30" maxlength="100" check="<?=get_lang('ly200.filled_out').get_lang('ly200.name');?>!~*"></td>
	</tr>
	<?php /*?><tr>
		<td nowrap><?=get_lang('translate.url');?>:</td>
		<td><input name="Url" type="text" value="<?=htmlspecialchars($translate_row['Url']);?>" class="form_input" size='60' maxlength="100"></td>
	</tr><?php */?>
	<?php if(count(get_cfg('ly200.lang_array'))>1){?>
		<tr>
			<td nowrap><?=get_lang('ly200.language');?>:</td>
			<td><?=output_language_select($translate_row['Language']);?></td>
		</tr>
	<?php }?>
