...
<table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table" not_mouse_trBgcolor_tr='list_form_title'>
	<tr align="center" class="list_form_title" id="list_form_title">
		...
		<?php if(get_cfg('instance.del')){?><td width="8%" nowrap><strong><?=get_lang('ly200.select');?></strong></td><?php }?>
		<?php if(get_cfg('instance.order')){?><td width="8%" nowrap><strong><?=get_lang('ly200.order');?></strong></td><?php }?>
	...
	<tr align="center">
		...
		<?php if(get_cfg('instance.mod')){?><td nowrap><a href="mod.php?<?=$all_query_string;?>&CaseId=<?=$instance_row[$i]['CaseId']?>"><img src="../images/mod.gif" alt="<?=get_lang('ly200.mod');?>"></a></td><?php }?>
	</tr>
	<?php }?>
