...
<div id="express_value" style="display:;">
	<?=htmlspecialchars($order_row['Express']);?>&nbsp;&nbsp;&nbsp;
	<?php if($order_product_weight==1){?>
		<?=get_lang('shipping.first_weight');?>: <?=$order_row['FirstWeight'];?> KG / <?=get_lang('ly200.price_symbols').$order_row['FirstPrice'];?>&nbsp;&nbsp;&nbsp;
		<?=get_lang('shipping.ext_weight');?>: <?=$order_row['ExtWeight'];?> KG / <?=get_lang('ly200.price_symbols').$order_row['ExtPrice'];?>&nbsp;&nbsp;&nbsp;
	<?php }?>
...
<div id="change_express_form" style="display:none;">
	<?=str_replace('<select', '<select id="ExpressSelect"', ouput_table_to_select('shipping', 'Express', 'Express', 'Express', 'MyOrder desc,SId asc', 1, 1, $order_row['Express']));?>&nbsp;&nbsp;
