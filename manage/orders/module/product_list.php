...
<table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table" not_mouse_trBgcolor_tr='list_form_title'>
	...
	<tr align="center">
		...
		<td align="left" class="flh_150">
			...
			<?php if($item_row[$i]['Color']){?><?=get_lang('product.color');?>: <?=$item_row[$i]['Color'];?><br /><?php }?>
			<?php if($item_row[$i]['Size']){?><?=get_lang('product.size');?>: <?=$item_row[$i]['Size'];?><br /><?php }?>
			<?php if($order_product_weight==1){?><?=get_lang('product.weight');?>: <?=$item_row[$i]['Weight'];?> KG<br /><?php }?>
	...
	<?php }?>
	<tr align="center">
		<td colspan="<?=get_cfg('orders.mod')?5:4;?>" nowrap align="left">
