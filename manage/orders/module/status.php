...
<table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table">
	<tr>
		...
		<td width="95%"><?=$order_row['PaymentMethod'];?></td>
	</tr>
	<?=$payment_info_detail;?>
	<tr>
	...
	<tr id="shipping_info_3" style="display:none;">
		...
		<td><?php if(get_cfg('orders.mod')){?><input type="text" name="TrackingNumber" id="tracking_number_input" value="<?=htmlspecialchars($order_row['TrackingNumber']);?>" size="20" maxlength="20" class="form_input" check_tmp="<?=get_lang('ly200.filled_out').get_lang('orders.tracking_number');?>!~*" /><?php }else{?><?=htmlspecialchars($order_row['TrackingNumber']);?><?php }?></td>
	</tr>
	<tr id="shipping_info_4" style="display:none;">
		<td nowrap><?=get_lang('orders.shipping_time');?>:</td>
		<td><?php if(get_cfg('orders.mod')){?><input name="ShippingTime" type="text" size="8" onclick="SelectDate(this);" contenteditable="false" value="<?=date('Y-m-d', $order_row['ShippingTime']?$order_row['ShippingTime']:$service_time);?>" class="form_input" /><?php }else{?><?=date('Y-m-d', $order_row['ShippingTime']);?><?php }?></td>
	</tr>
...
<script language="javascript">change_order_status(<?=$tmpOrderStatus?$tmpOrderStatus:$order_row['OrderStatus'];?>);</script>
</form>