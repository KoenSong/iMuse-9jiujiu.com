...
if($_POST['list_form_action']=='member_del'){
	...
	if(count($_POST['select_MemberId'])){
		...
		$db->delete('product', "MemberId in($MemberId)");
	}
...
$IsTeacher = (int)$_GET['IsTeacher'];
$Apply = (int)$_GET['Apply'];

$FullName && $where.=" and m.UserName like '%$FullName%'";
$Phone && $where.=" and m.Phone like '%$Phone%'";
if($IsTeacher==1){
	$where.=" and m.IsTeacher = 0";
}elseif($IsTeacher==2){
	$where.=" and m.IsTeacher = 1";
}
$Apply && $where.=" and m.Apply =1";
...
$row_count=$db->get_row_count('member m', str_replace('m.','',$where));
$total_pages=ceil($row_count/get_cfg('member.page_count'));
$page=(int)$_GET['page'];
$page<1 && $page=1;
$page>$total_pages && $page=1;
$start_row=($page-1)*get_cfg('member.page_count');
$coco=get_cfg('member.page_count');
$member_rs=$db->query("select m.*, sum(TotalPrice) as total_price from member m left join (select * from orders where OrderStatus in(5, 6)) o on m.MemberId=o.MemberId where $where group by m.MemberId order by {$order_ary[$order]}m.MemberId desc limit $start_row,$coco");
	...
	<tr>
		<td height="22" class="flh_150">
			<form method="get" name="manage_log_search_form" action="index.php" onsubmit="this.submit.disabled=true;">
				...
				手机:<input name="Phone" class="form_input" type="text" size="20" maxlength='200'>
                ...
                审核:<select name="Apply">
                		<option>-- 请选择 --</option>
                		<option value="1">审核</option>
               		</select>
               	会员类型:<select name="IsTeacher">
                    <option>-- 请选择 --</option>
...
<table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table" not_mouse_trBgcolor_tr='list_form_title'>
	<tr align="center" class="list_form_title" id="list_form_title">
		...
		<?php /*?><td width="5%" nowrap><strong><?=get_lang('member.consumption_price');?></strong><a href="index.php?<?=$query_string_no_order;?>&order=5"><img src="../images/asc.png" /></a><a href="index.php?<?=$query_string_no_order;?>&order=6"><img src="../images/desc.png" /></a></td><?php */?>
		<td width="5%" nowrap><strong><?=get_lang('member.login_times');?></strong><a href="index.php?<?=$query_string_no_order;?>&order=7"><img src="../images/asc.png" /></a><a href="index.php?<?=$query_string_no_order;?>&order=8"><img src="../images/desc.png" /></a></td>
	...
	<tr align="center">
		...
		<td nowrap><?=htmlspecialchars($member_row['Title']);?></td>
		<td nowrap><?=htmlspecialchars($member_row['UserName']);?></td>
		<td nowrap><?=htmlspecialchars($member_row['Phone']);?></td>
        <td nowrap><?php echo sprintf('%08s',$member_row['MemberId'])?></td>
		<td nowrap><?=date(get_lang('ly200.time_format_full'), $member_row['RegTime']);?></td>
		<td><?=htmlspecialchars($member_row['RegIp']);?> [<?=$reg_ip_area['country'].$reg_ip_area['area'];?>]</td>
		<td nowrap><?=date(get_lang('ly200.time_format_full'), $member_row['LastLoginTime']);?></td>
		<td><?=htmlspecialchars($member_row['LastLoginIp']);?> [<?=$last_login_ip_area['country'].$last_login_ip_area['area'];?>]</td>
		<?php /*?><td nowrap><?=get_lang('ly200.price_symbols').sprintf('%01.2f', $member_row['total_price']);?></td><?php */?>
		<td nowrap><?=$member_row['LoginTimes'];?></td>
		<td><a href="view.php?<?=$query_string;?>&MemberId=<?=$member_row['MemberId']?>"><img src="../images/view.gif" alt="<?=get_lang('ly200.view');?>" /></a></td>
	</tr>
	<?php }?>
