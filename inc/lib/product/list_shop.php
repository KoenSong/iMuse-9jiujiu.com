...
<div id="lib_product_list_shop">
	...
	for($i=0; $i<count($product_row); $i++){
			...
			$meet_calss_Count && $Continue_rate=ceil($db->get_row_count('order_twos',"ProId ='{$product_row[$i]['ProId']}' and OrderStatus = '3'")/$meet_calss_Count*100);
			...
			$review_rate='0';
			$review_all && $review_rate=ceil($review_good/$review_all*100);
	...
	?>
			<div class="item_0 translate round">
				<ul>
					<li class="img translate round" style="width:<?=$product_img_width;?>px; height:<?=$product_img_height;?>px;">
					...
					<li class="info">
                        ...
                        <div class="teacher_par">
							...
							<span>
								<?=$db->get_value('product_color',"CId='{$product_row[$i]['ColorId']}'",'Color')?>
								<?=$db->get_value('product_circle',"CId='{$product_row[$i]['CircleId']}'",'Circle') == '' ? '' : '- '.$db->get_value('product_circle',"CId='{$product_row[$i]['CircleId']}'",'Circle')?>
							</span>                        
						</div>
					...
					<li class="price">
                    	<div class="review_star">
							<?php /*?><?php 
								if($review_sum){
								for($t=0;$t<=ceil($review_sum/$review_all);$t++){
							?>
                        ...
                        <div class="P_1">
                        	<span>
                        		<!-- <?=$product_row[$i]['Price_1']?> -->
                        		<?php
                        		!empty($product_row[$i]['Price_1']) && $product_row[$i]['Price_1'] != 0 && $priceArr[] = $product_row[$i]['Price_1'];
                        		!empty($product_row[$i]['Price_2']) && $product_row[$i]['Price_2'] != 0 && $priceArr[] = $product_row[$i]['Price_2'];
                        		!empty($product_row[$i]['Price_3']) && $product_row[$i]['Price_3'] != 0 && $priceArr[] = $product_row[$i]['Price_3'];
                        		$idx = array_search(min($priceArr), $priceArr);
                        		echo($priceArr[$idx]);
                        		$selPrice = $priceArr[$idx];
                        		unset($priceArr);
                        		?>
                        	<span>
                        	<font>元/小时</font>
                        </div>
                        <div class="Detail round"><a href="<?=$url."&selPrice=".$selPrice?>"></a></div>
					</li>
				</ul>
			</div>
