...
<div id="lib_product_list">
	...
	<div class="item" style="width:<?=sprintf('%01.2f', 100/$list_row_count);?>%;">
		<ul style="width:<?=$product_img_width+2;?>px;">
			<li class="img" style="width:<?=$product_img_width;?>px; height:<?=$product_img_height;?>px"><div class="PicPath" style="width:<?=$product_img_width;?>px; height:<?=$product_img_height;?>px"><a href="<?=$url;?>" target="_blank"><img src="<?=$product_row[$i]['PicPath_0'];?>"/></a>
            <span></span>
            </div></li>
			<li class="name"><a href="<?=$url;?>" target="_blank"><?=$product_row[$i]['Name'];?></a></li>
		</ul>
	</div>
...
<?php
$product_list_lang_0=ob_get_contents();
ob_clean();
?>