...
if($_POST['data']=='member_add_issue'){
	...
	if($Date !='|'){
		//商家账户
		if($ProId){
			if($db->get_row_count('product',"ProId = '{$ProId}'")){
			...
			if($ProId)
				js_location("$member_url_cn?module=issue_mod&ProId=".$ProId,'修改成功!');
			else
		...
		if($ProId)
			js_location("$member_url_cn?module=issue_mod&ProId=".$ProId,'添加失败!');
		else
