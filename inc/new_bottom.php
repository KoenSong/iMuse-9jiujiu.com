<?php 
//$where='IsTeacher = 1';
$member_rs=$db->query("select m.ApplyTime,m.MemberId from member_apply m left join (select MemberId,IsTeacher from member where IsTeacher=1) o on m.MemberId=o.MemberId where m.Ischeck=1 and o.IsTeacher=1 order by m.ApplyTime desc limit 0,6");
$new_c_row=array();
...
<?php $t=0;foreach((array)$new_c_row as $item){
    ...
    <div class="item">
        ...
        <div class="r">
            ...
            <div class="time1"><?=date('Y-m-d',$item['ApplyTime'])?></div>
        </div>
    </div>
