...
if($_POST['data']=='member_login'){
	...
	$jump_url=(substr($jump_url, 0, 1)!='/' || substr_count($jump_url, 'logout'))?"/weixin/web"."$member_url_cn?module=orders&act=prelist#contents":$jump_url;
	if($Phone=='' || $_POST['Password']==''){
		...
		//登录成功
		//约课页面跳转标志
		$get_forward = $_POST['forward'];
		$get_ProId = $_POST['ProId'];
		if(isset($get_forward) && isset($get_ProId) && $get_forward != '' && $get_ProId != ''){
			$jump_url = $get_forward."?ProId=".$get_ProId;
		}
	...
	if($Date !='|'){
		if($db->get_row_count('product',"MemberId = '{$member_info['MemberId']}'")){
			...
			if($Intro_Type==2){
				...
				//$Warranty3 = $_POST['T_success'];
				//$Warranty4 = format_post_value($_POST['T_ter']);
				$Warranty1 = $_POST['T_gift'];
				$Warranty2 = $_POST['T_thought'];
				$Warranty3 = $_POST['T_success'];
				$Warranty4 = $_POST['T_ter'];
			}else{
