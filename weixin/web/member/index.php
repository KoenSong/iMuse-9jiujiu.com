...
if($member_info['IsTeacher']){
	$login_module_ary=array('index', 'profile', 'password', 'orders','order_twos', 'wishlists', 'logout','face_mod','addprice','apply','issue_mod','price','ident','issue_mod','review','withdraw','qrcode');	//需要登录的模块列表
}elseif($member_info['Apply']){	
	$login_module_ary=array('index', 'profile', 'password', 'orders','order_twos', 'wishlists', 'logout','face_mod','addprice','price','review','ident','issue_mod','withdraw','qrcode');	//需要登录的模块列表
}else{
...
if((int)$_SESSION['member_MemberId']){	//已登录的，架构会员中心页面内容排版
	...
	if($member_info['Apply']){
	$account_url='/weixin/web/products-detail.php?ProId='.$db->get_value('product',"MemberId = '{$member_info['MemberId']}'",'ProId');
	}else{
		$account_url="/weixin/web/student.php?MemberId={$member_info['MemberId']}";
	}
?>
<div class="blank30"></div>
<div id="contents" class="w1200 over">
    <div class="lft fl">
    	<?php include($site_root_path."/weixin/web/member/module/menu.php");?>
    </div> <!-- 左侧 -->
    
    <div class="rig fr over">
        <div class="member_module">
        	<!--<?php include($site_root_path."/web/lib/member/module/$module.php");?>-->
        	<?php include($site_root_path."/weixin/web/member/module/$module.php");?>
        </div>
        </div>
    </div> <!-- 右边 -->
</div>
...
<?php
	...
	include($site_root_path."/weixin/web/member/module/$file_base_dir/$module.php");
}
